...
const Multitransactions = () => {
  return (
    ...
    quoteThree={'"A big part of our service is with regular customers"'}/><SpacerLarge/>
    <Paragraph>Multi-transactions was a hygiene feature, our users expected it to exist so this particular project did not require a full-scale discovery. I did however seek evidence to confirm this assumption by interviewing both buyers and sellers.</Paragraph><SpacerXSmall/>
    <Paragraph>An unexpected insight was that buyers wanted to see the total amount of credit they could be approved for, rather than being assessed for every invoice. This also appealed to sellers as a way to increase cart value.</Paragraph><SpacerLarge/>
    <SingleImage fit={"contain"} height={900} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={MultiJourney}/><SpacerLarge/>
    <Heading>Development</Heading><SpacerSmall/>
    ...
    <Paragraph>The challenges were; figuring out the hierarchy of information on the dashboard - to show the most important top-level information first and make sure users could also deep dive into the details of transactions for reconciliation and tracking.</Paragraph><SpacerXSmall/>
    <Paragraph>We ran a card sorting exercise to help us understand what was most important to our users. Additionally, we knew that many customers were sharing their dashboard with their accountants, so we repeated the same card-sorting exercise with this group of users.</Paragraph><SpacerLarge/>
    <SingleImage fit={"contain"} height={500} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={MultiExplainer}/><SpacerLarge/>
    <Heading>Low fidelity</Heading><SpacerSmall/>
    <Paragraph>Emulating the look and feel of a paper invoices - I opted for a card system instead of numerical tables. Progress visualisations were added to encourage repayments and reduce buyer’s remorse.</Paragraph><SpacerXSmall/>
    <Paragraph>Multiple repayment brought up the question of whether to consolidate them to a single schedule - but, we know from behavioural insights that users are engaged in activities with smaller achievements - so the payments were kept ‘attached’ to the original transaction to make them feel more manageable.</Paragraph><SpacerLarge/>
    <TripleGallery 
    color={"#F9F9F9"} 
    ...
    /><SpacerLarge/>
    <SubHeading>Dashboard header system</SubHeading><SpacerXSmall/>
    <Paragraph>The high-fidelity designs made use of a pattern system for the top banner, for every different dashboard state, the user is greeted with a slightly different look & feel.</Paragraph><SpacerXSmall/>
    <Paragraph>This was a way of communicating if they are overdue or paid off their balance without relying on copy.</Paragraph><SpacerLarge/>
