...
const InstantAccess = () => {
  return (
    <div>
    <Heroimage position={"center"} padding={0} image={InstantAccessHero} color={"#33885B"}/>
    ...
    <Callout>SME's struggle to see how iwocaPay fits their business - "iwocaPay looks great, but, it's not for us"</Callout><SpacerSmall/>
    <Paragraph>We had a 50% increase in our traffic to the website, but conversion didn’t improve. So, what were these people looking for that they didn’t find? How can we better tailor iwocaPay or talk about iwocaPay so sellers can more quickly evaluate whether it’s a solution for them?</Paragraph><SpacerLarge/>
    <SingleImage fit={"contain"} height={500} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={MobileIA}/><SpacerLarge/>
    <Heading>Evidence</Heading><SpacerSmall/>
    <Paragraph>In the first year of iwocaPay we were able to capture the industry of our merchants because of the manual onboarding. We saw some initial data to suggest industry segmented behaviour ( although numbers are too low to tell ) - part of this project was to recapture more business information at some point in the journey.</Paragraph><SpacerXSmall/>
    ...
    explainerTwo={"These insights highlight the way businesses have been experimenting and trying new products. It’s pretty clear self-serve has become very common place and having to call in or wait to use the product may cause friction."}/><SpacerLarge/>
    <Heading>User journey</Heading><SpacerSmall/>
    <Paragraph>There was plenty of discussion in figuring out when to ask which questions. The 'Director's profile' are the compulsory details we need to check a potential business. In this new flow we're requesting additional questions as a way to understand our users and prioritise leads.</Paragraph><SpacerXSmall/>
    <Paragraph>It's the first time users will be able to upload bank statements and pass Onfido checks without assistance - and, we wouldn't need to send a contract for the 'Seller agreement' via email any longer, users will be able to agree to this via a tickbox.</Paragraph><SpacerLarge/>
    <SingleImage fit={"contain"} height={500} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={IAExplainer}/><SpacerXSmall/>
    <SingleImage fit={"contain"} height={1000} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={IAUsermap}/><SpacerLarge/>
    <Heading>Low Fidelity</Heading><SpacerSmall/>
    <Paragraph>For this project - we needed a simple solution that could be implemented quickly and easily customisable in the future.</Paragraph><SpacerXSmall/>
    <Paragraph>The two interface options were an expandable activation modal, that would follow the User around the account whilst they explored features VS a new 'global dashboard' that would change states from a sign up screen to an activated account state.</Paragraph><SpacerLarge/>
    <FourByFourGrid
    color={"#F9F9F9"} 
    ...
    captionFour={"Once the activation state is complete it would show a User an overview of their account"}/><SpacerLarge/>
    <Heading>Testing</Heading><SpacerSmall/>
    <Paragraph>After speaking with developers - both options would require the same amount of effort. I created an initial prototype for Option 2 - a new global dashboard, to test with users.</Paragraph><SpacerXSmall/>
    <Paragraph>The goal for the test was to understand if potential sellers were able to see how iwocaPay fits their business through exploring it’s features.</Paragraph><SpacerLarge/>
    <SingleImage fit={"cover"} height={500} marginTop={0} marginBottom={0} color={"#F9F9F9"} image={IABottomFrame}/><SpacerSmall/>
    <FullBleedGif marginTop={100} marginBottom={100} color={"#33885B"} image={IAGif} width={830} height={519}/><SpacerLarge/>
    <Heading>Responses</Heading><SpacerSmall/>
    <Paragraph>The main consideration when looking for a new payments solution was fees.</Paragraph><SpacerXSmall/>
    <Quote>"So, cost obviously - what’s it going to cost and what’s going to get taken away from any invoices that are paid"</Quote><SpacerXSmall/>
    <Quote>"cost is a big one..integrations too, but the cost is going to be a big one”</Quote><SpacerXSmall/>
    <Paragraph>It was clear that the users who played around with the Pay links page for longer understood the product.</Paragraph><SpacerXSmall/>
    <Quote>"Ahh Paylinks - yeah! If this goes live - I wanna know about it! Yeah all the links is perfect...well, this is even better I can see the different options across the top - I have the option of seeing who pays - for this. Okay and the Pay Later option I can choose here who pays for that privilege."</Quote><SpacerMedium/>
    <InlineImage image={InlineImageIA} imageHeight={368} imageWidth={469}/><SpacerMedium/>
    <Paragraph>I run an initiative called 'UserTV' which is a tick-tock-style slack channel of User insights.</Paragraph><SpacerLarge/>
    <FullBleedGif marginTop={50} marginBottom={50} color={"#F9F9F9"} image={IAGiftwo} width={830} height={519}/><SpacerLarge/>
    <Heading>High fidelity</Heading><SpacerSmall/>
    <Paragraph>After implementatons there was a significant increase in both conversion rate (by 2%) and the speed of onboarding (it took on average under a day, compared to a week prior to the release).</Paragraph><SpacerXSmall/>
    ...
    /><SpacerSmall/>
    <FullBleedGif marginTop={50} marginBottom={50} color={"#33885B"} image={IAGifthree} width={830} height={519}/>
    </div>
  )
}
